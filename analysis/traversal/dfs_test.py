...
def test_dfs(recursive: bool) -> None:
    ...
    # empty graph
    g = Graph()
    pre, post = dfs(g, recursive=recursive)
    assert pre == []
    assert post == []
    ...
    for n in (2, 3, random.randint(4, MAX_TEST_GRAPH_SIZE)):
        ...
        pre, post = dfs(g, recursive=recursive, seed_order=Order.SORTED)
        assert pre == list(range(n)), n
        assert post == pre, n
        # seeds in reverse sorted order
        pre, post = dfs(g, recursive=recursive, seed_order=Order.REVERSE_SORTED)
